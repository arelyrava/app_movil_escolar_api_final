    ...
    def get(self, request, *args, **kwargs):
        ...
        for maestro in lista:
            if isinstance(maestro, dict) and "materias_json" in maestro:
                try:
                    # Deserializar el JSON de las materias
...
class MaestrosView(generics.CreateAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not maestro_id:
            return Response({"detail": "ID del maestro requerido."}, status=status.HTTP_400_BAD_REQUEST)
            
        maestro = get_object_or_404(Maestros, id = maestro_id)
        maestro_data = MaestroSerializer(maestro, many=False).data
        
        # Deserializar el JSON de las materias antes de enviarlo al frontend
    ...
    def put(self, request, *args, **kwargs):
        ...
        # Guardamos el JSON de materias serializado como string
